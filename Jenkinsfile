            ...
            steps {
                withCredentials([
                    ...
                    sh '''#!/bin/bash
                        ...
                        echo "=> Получение учетных данных из Vault..."
        ...
        stage('Build & Push Docker Image') {
            steps {
                sh '''#!/bin/bash
                    ...
                    echo "=> Установка Docker Buildx для умного кэширования..."
                    mkdir -p ~/.docker/cli-plugins
                    if [ ! -f ~/.docker/cli-plugins/docker-buildx ]; then
                        curl -sLo ~/.docker/cli-plugins/docker-buildx https://github.com/docker/buildx/releases/download/v0.11.2/buildx-v0.11.2.linux-amd64
                    ...
                    echo "=> Сборка образа (BuildKit)..."
                    docker --tlsverify --tlscacert=ca.crt --tlscert=client.crt --tlskey=client.key build \
